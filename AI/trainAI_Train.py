...
DS_path = 'gdrive/My Drive/trainAI_FS/'
OUT_Path = 'gdrive/My Drive/trainAI_FS/OUT/'
ANNO_file = 'gdrive/My Drive/trainAI_FS/CVAT.xml'

print(open(DS_path+'test.txt','r').read())
...
def get_date_XML(xml_path):

    anno_file = ET.parse(xml_path)
    root = anno_file.getroot()
    ...
    for img in root.findall("./image"):
        ...
        for box in img.findall("./box"):
            ...
            if filename not in all_imgs:
                ...
                img = cv2.imread(DS_path+'Trackpictures_LoRes/'+filename)
                (rows, cols) = img.shape[:2]
                all_imgs[filename]['filepath'] = DS_path+'Trackpictures_LoRes/'+filename
...
output_weight_path = os.path.join(OUT_Path, 'model\\model_frcnn_vgg.hdf5')

record_path = os.path.join(OUT_Path,'model\\record.csv')  # Record data (used to save the losses, classification accuracy and mean average precision)

base_weight_path = os.path.join(OUT_Path, 'model\\vgg16_weights_tf_dim_ordering_tf_kernels.h5')

config_output_filename = os.path.join(OUT_Path, 'model_vgg_config.pickle')
#
# Create the config
C = Config()
...
st = time.time()
train_imgs, classes_count, class_mapping = get_date_XML(ANNO_file)
print()
...
saved_model = pickle.dumps(C)
open('model_vgg_config.pickle','wb').write(saved_model)
...
tpu_model = tf.contrib.tpu.keras_to_tpu_model(
    model_all,
    strategy=tf.contrib.tpu.TPUDistributionStrategy(
        tf.contrib.cluster_resolver.TPUClusterResolver(TPU_ADD)))
...
tpu_model.compile(optimizer='sgd', loss='mae')
        ...
        try:
            ...
            if iter_num == epoch_length:
                ...
                if curr_loss < best_loss:
                    ...
                    best_loss = curr_loss
                    tpu_model.save_weights(C.model_path)
