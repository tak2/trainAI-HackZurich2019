...
#add deviceID and type to datafram
df['did'] = 0
df['type'] = 0
df['locabb'] = 0
...
for index, row in df.iterrows():
        ...
        if index != 0: 
            ...
            #row.dist = 1
            tempstr = tsddf.loc[tsddfinx ][4 ]
            tempstr = tempstr[:-3]
            #print( index ,round(disttot / 1000 ,2))
            #print (float(round(disttot / 1000 ,2)) , float(tempstr))
            #print(abs(float(round(disttot / 1000 ,2))-float(tempstr))<0.00001)
            #print (int(float(tempstr)*1000))
            if round(disttot ,2) > int(float(tempstr)*1000) :
                tsddfinx = tsddfinx +1
                print("yes")
                df.loc[index,'did'] = tsddf.loc[tsddfinx][1 ]
                df.loc[index,'type'] = tsddf.loc[tsddfinx ][2 ]
                df.loc[index,'locabb'] = tsddf.loc[tsddfinx ][7 ]

        lstlon = row[1]
        lstlat = row[2]
df = df[df.did != 0]
df.to_csv("file301.csv",index=False)
